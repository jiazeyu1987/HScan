...
const HospitalsPage: React.FC = () => {
  ...
  const stats = {
    total: total,
    active: Array.isArray(hospitals) ? hospitals.filter(h => h.status === 'active').length : 0,
    verified: Array.isArray(hospitals) ? hospitals.filter(h => h.verified).length : 0,
    withWebsite: Array.isArray(hospitals) ? hospitals.filter(h => h.website_url).length : 0
  }
